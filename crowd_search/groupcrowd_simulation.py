...
class GroupCrowdSim(gym.Env):
    """GroupCrowd simulation environment."""
    ...
    def __init__(
        ...
        self.group_num = env_cfg.get("num-groups")
    ...
    def _generate_groups(self ) -> None:
        ...
        """
        # Create initial groupagent.
        # num_groups= random.randint(0, max_num_groups-1)
        self.humans= []
        self.groups = []
        for i in range(self.group_num):
           group= agents.Group(i, self.group_cfg)
           for j in range(group.num_of_agents):
               human = agents.Human(self.human_cfg)
               human.set_group_id(i)
               while True:
                   positionx = round(random.uniform(group.start_positions[0]-1.5,
                                      group.start_positions[0]+1.5), 1)
                   positiony = round(random.uniform(group.start_positions[1]-1.5,
                                      group.start_positions[1]+1.5), 1)
               ...
               velocity_x=0.0,
               velocity_y=0.0,
               goal_position_x=random.uniform(group.goal_positions[0]-2.0
                                       ,group.goal_positions[0]+2.0),
               goal_position_y=random.uniform(group.goal_positions[1]-2.0
                                       ,group.goal_positions[1]+2.0),
               direction=0.0,
               )
